export function normalizeTo3DMatrix(matrix) {
...
export function parseCSSTransform(transformString) {
 ...
 */
export function getMatrix3D(transformations = [], origin = { x: 0, y: 0 }) {
...
export function has3Dtransforms(transforms, force3D = false) {
 ...
 */
export function canFlattenTo2D(matrix) {
 ...
 */
export function flattenTo2D(matrix, perspective = Infinity) {
 ...
 */
export function qrDecomposeMatrix(matrix, precision = 3) {
...
export function analyzeMatrix(matrix) {
    ...
    let matrixProper3D = getProper3DMatrix(matrix)
...
export function getSkewX(matrix, precision = 3) {
...
export function getSkewY(matrix, precision = 3) {
    if (typeof matrix !== "object" || matrix === null) {
        throw new Error("Invalid matrix. Expected an object with a-p properties.");
    }
...
export function getScaling(matrix, precision = 3) {
    if (typeof matrix !== "object" || matrix === null) {
        throw new Error("Invalid matrix. Expected an object with a-p properties.");
    }
...
export function getAdjustedSkew(matrix, skewX = 0, skewY = 0, scaleX = 1, scaleY = 1, scaleZ = 1, precision = 9) {
    if (typeof matrix !== "object" || matrix === null) {
        throw new Error("Invalid matrix. Expected an object with a-p properties.");
    }
...
export function qrDecomposeMatrix3D(matrix, precision = 3) {
    if (typeof matrix !== "object" || matrix === null) {
    ...
    let r00 = a / scaleX, r01 = b / scaleX, r02 = c / scaleX;
    let r10 = e / scaleY, r11 = f / scaleY, r12 = g / scaleY;
    let r20 = i / scaleZ, r21 = j / scaleZ, r22 = k / scaleZ;
  

    // **Step 4: Improved Skew and Rotation Detection**
    let relativeThreshold = 1e-4; // Relative threshold for detection
    ...
    */
    ...
    if (has_skewX) {
        ...
        //console.log('has_skewX', skewX);

        // Adjust skewX for rotation effects
        if (has_rots) {
            //skewX -= rotateZ; // Compensate for Z rotation
        }
    }
    ...
    if (has_skewY) {
        ...
        skewY = skewYAdj
        //console.log('has_skewY', skewYAdj);

        // Adjust skewY for rotation effects
        if (has_rots) {
           //skewY -= rotateY; // Compensate for Y rotation
        }
    }
...
export function getProper3DMatrix(matrix, perspectiveOrigin = { x: 0, y: 0 }, transformOrigin = { x: 0, y: 0 }, perspective = Infinity) {
    // Clone base matrix
...
export function transformPointByProper3DMatrix(matrix, pt) {
